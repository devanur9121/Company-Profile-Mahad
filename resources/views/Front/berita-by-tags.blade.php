      ...
      <div class="col-lg-12 my-auto">
        <div class="page-banner-content text-center">
          ...
          <div class="page-banner-breadcrumb">
            <p><a href="/home">Home</a> Blog by Tags</p>
          </div>
        </div>
      </div>
    </div>
          ...
          <div class="col-12">
            <div class="site-pagination">
              <div class="navbar justify-content-center">
                <ul class="pagination">
                  @if($news_tags->onFirstPage())
                  <li class="page-item disabled"><span class="page-link" aria-hidden="true">&laquo;</span></li>
                  @else
                  <li class="page-item"><a class="page-link" href="{{ $news_tags->previousPageUrl() }}" rel="prev"
                      aria-label="Previous">&laquo;</a></li>
                  @endif

                  @foreach(range(1, $news_tags->lastPage()) as $page)
                  @if($page == $news_tags->currentPage())
                  <li class="page-item active"><span class="page-link">{{ $page }}</span></li>
                  @else
                  <li class="page-item"><a class="page-link" href="{{ $news_tags->url($page) }}">{{ $page }}</a>
                  </li>
                  @endif
                  @endforeach

                  @if($news_tags->hasMorePages())
                  <li class="page-item"><a class="page-link" href="{{ $news_tags->nextPageUrl() }}" rel="next"
